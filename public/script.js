...
function renderProgress(progress) {
  return progress.map(p => `
    <div class="progress-item p-2 border-b">
      <p>${p.text}</p>
      ${p.imageUrl ? `<a data-fslightbox href="${p.imageUrl}"><img src="${p.imageUrl}" alt="Progress Image" class="w-16 h-16 object-cover rounded-md mt-2"></a>` : ''}
      <div class="text-xs text-gray-500 mt-1">由 <strong>${getDisplayName(p.creatorId)}</strong> 在 ${formatDate(p.createdAt)} 添加</div>
    </div>
  `).join('');
}
